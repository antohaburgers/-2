...
async function start(){await RAPIER.init();world=new RAPIER.World({x:0,y:0});
  ...
  function respawn(section){input.reset();car.body.setTranslation({x:section.x,y:section.y},true);car.body.setRotation(section.a,true);car.body.setLinvel({x:0,y:0},true);car.body.setAngvel(0,true);for(const key of ['forwardSpeed','sideSpeed','yawRate','wheelAngle','driftAmount','slip'])car[key]=0;car.drift=false;car.rearLocked=false;car.setInput(input.get());particles=new Particles();camera.x=section.x;camera.y=section.y;accumulator=0;audio.gear=1;audio.rpm=950;}
  ...
  function frame(now){
    ...
    draw();debug.textContent=`V6 · ${Math.abs(s.speed*3.6).toFixed(0)} км/ч${s.speed<-.2?' · R':''}\n${nearestSection(s.x,s.y).name}\n${s.rearLocked?'РУЧНИК · БЛОК':s.drift?'DRIFT':''}`;
