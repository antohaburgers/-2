...
if('serviceWorker' in navigator && location.protocol!=='file:'){
  ...
  navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'none'}).then(reg=>{
    ...
    offer();reg.addEventListener('updatefound',()=>{const worker=reg.installing;worker?.addEventListener('statechange',()=>{if(worker.state==='installed'){if(reg.waiting&&navigator.serviceWorker.controller)offer();else status.textContent='V9 · сохранено для офлайн-игры';}if(worker.state==='redundant')status.textContent='Офлайн-копия не сохранена. Открой игру с интернетом.';});});
    navigator.serviceWorker.ready.then(()=>{if(!reg.waiting)status.textContent='V9 · сохранено для офлайн-игры';});
