...
export const CAR_CONFIG = {
  ...
  throttleDrift: 2.8, driftBuild: 4.8, gripRecoverySpeed: 1.05,
...
export class CarPhysics {
  ...
  step(dt) {
    ...
    const steerLoad=Math.abs(this.wheelAngle)*clamp(absForward/2.6,0,1);
    ...
    if(this.rearLocked) this.forwardSpeed=approach(this.forwardSpeed,0,3.2*dt);
    const rearGrip=this.rearLocked ? .04 : lerp(c.rearGrip,c.handbrakeGrip,this.driftAmount);
    ...
    const steeringPush=-this.forwardSpeed*Math.sin(this.wheelAngle)*(this.driftAmount*1.65+.055);
    this.sideSpeed+=steeringPush*dt;
    this.sideSpeed=approach(this.sideSpeed,0,(rearDecay+frontDecay*(1-this.driftAmount*.8))*dt);
    // Signed sideslip aligns the body with its travel direction; steering still owns the turn.
    const signedSlip=Math.atan2(this.sideSpeed,Math.max(2,absForward));
    const driftYaw=geometricYaw*(1+this.driftAmount*.24)+signedSlip*1.45;
    const yawTarget=lerp(geometricYaw,driftYaw,this.driftAmount);
    const correcting=this.steerInput*this.yawRate<0;
    this.yawRate=approach(this.yawRate,yawTarget,(correcting?5.8:lerp(c.yawResponse,2.8,this.driftAmount))*dt);
    const sideLimit=Math.max(.3,absForward*.8);
    this.sideSpeed=clamp(this.sideSpeed,-sideLimit,sideLimit);
  ...
  getState() { const t=this.body.translation(), rearLeft=this.point(-this.cfg.wheelBase/2,-this.cfg.track/2), rearRight=this.point(-this.cfg.wheelBase/2,this.cfg.track/2); return {x:t.x,y:t.y,angle:this.body.rotation(),wheelAngle:this.wheelAngle,speed:this.forwardSpeed,sideSpeed:this.sideSpeed,slip:this.slip,drift:this.drift,driftAmount:this.driftAmount,rearLocked:!!this.rearLocked,throttle:this.throttle,rearWheels:[rearLeft,rearRight]}; }
}
export { RAPIER };
