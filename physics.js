...
export class CarPhysics {
  ...
  step(dt) {
    ...
    if(this.rearLocked) travel=approach(travel,0,4.0*dt);
    // Exponential grip recovery cannot erase small lateral velocities in one step.
    const grip=this.rearLocked?.16:lerp(7,.72,this.driftAmount);
    ...
    if(handbrakePressed&&Math.abs(travel)>1.5){
      const turn=Math.abs(this.wheelAngle)>.025?Math.sign(this.wheelAngle)*direction:Math.abs(this.yawRate)>.15?Math.sign(this.yawRate):0;
      this.yawRate+=turn*.9*clamp(Math.abs(travel)/4,0,1);
    }
