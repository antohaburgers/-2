...
export class CarPhysics {
  ...
  step(dt) {
    ...
    const slowTurn=clamp((7-absForward)/4,0,1)*clamp(absForward/1.7,0,1);
    const maxSteer=c.steeringAngle*(1-c.highSpeedSteeringReduction*speedNorm)+slowTurn*.125;
    ...
    if(this.rearLocked) travel=approach(travel,0,8.2*dt);
    // Exponential grip recovery cannot erase small lateral velocities in one step.
    const grip=this.rearLocked?.04:lerp(7,.72+slowTurn*.28,this.driftAmount);
    ...
    const yawTarget=geometricYaw*(1+this.driftAmount*(.22+slowTurn*.25))*(this.rearLocked?1.22:1)+alignment;
