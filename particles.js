export class Particles {
  ...
  update(dt, car, state) {
    ...
    const rearX = state.x - Math.sin(a) * .825, rearY = state.y + Math.cos(a) * .825;
    if(s>.1) { for(let i=0;i<Math.ceil(s*3);i++){ const side=i%2?1:-1; const x=rearX+Math.cos(a)*side*.42, y=rearY+Math.sin(a)*side*.42; this.smoke.push({x,y,vx:(Math.random()-.5)*.5,vy:(Math.random()-.5)*.5,r:.12+Math.random()*.2,life:.55+Math.random()*.5}); } }
    if(s>.28) { const side=Math.random()>.5?1:-1; this.trails.push({x:rearX+Math.cos(a)*side*.42,y:rearY+Math.sin(a)*side*.42,a,life:5}); }
