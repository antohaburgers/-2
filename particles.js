export class Particles {
  ...
  update(dt,state){
    const intensity=Math.min(1,state.driftAmount*.75+state.slip/28)*(state.drift?1:0);
    if(intensity>.12 && this.lastWheels) for(let i=0;i<2;i++) this.trails.push({a:this.lastWheels[i],b:{...state.rearWheels[i]},life:7});
    this.emit-=dt;
